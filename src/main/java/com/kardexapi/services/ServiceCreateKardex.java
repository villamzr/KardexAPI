...
import com.kardexapi.dto.KardexCreate;
...
public class ServiceCreateKardex {
	...
	private ResponseEntity<KardexCreate> restTemplateResult;
	...
	public RestTemplate consume(String url, Map<String, String> map) {
		...
		try {
			...
			setRestTemplateResult(restTemplate.postForEntity(url, request, KardexCreate.class));
		} catch (Exception e) {
	...
	public ResponseEntity<KardexCreate> getRestTemplateResult() {
		return restTemplateResult;
	}

	public void setRestTemplateResult(ResponseEntity<KardexCreate> restTemplateResult) {
